...
2. `docs/design.md`.
3. Существующий код этого репозитория.
...
Исходный код приложения `diasync-backend` доступен локально уровенем выше данного проекта. Его
...
- Запрещено передавать Room entities и Retrofit DTO напрямую в widget/watch/domain presentation.
...
Если код содержит очевидную платформенную ошибку, race или утечку lifecycle, сохрани
...
- Не обновляй AGP/Gradle/targetSdk без необходимости.

## Рабочий процесс

Перед изменениями:

1. Прочитай `docs/design.md` и этот файл.
...
3. Если реализация изменила архитектурное решение, обнови `docs/design.md` в том же change.
4. В финальном ответе перечисли: результат, важные файлы, выполненные проверки, что не удалось
   проверить. Предложи свой вариант commit message на английском языке.
