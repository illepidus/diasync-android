...
Для версии `X.Y` workflow создаёт ветку `diasync-release-X.Y`, запускает `./gradlew test`, собирает
три подписанных APK и публикует GitHub Release `vX.Y`. Если запуск завершается с ошибкой,
