...
Видимое LOW/HIGH notification снимается, когда текущее значение возвращается за соответствующий
порог или этот тип алерта отключается. NO DATA notification снимается после получения свежей точки
или отключения этого типа алерта. Ограничение повторов и snooze сами по себе notification не
снимают, пока соответствующее ненормальное состояние сохраняется.
